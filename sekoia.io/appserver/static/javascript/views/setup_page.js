...
define(["backbone", "jquery", "splunkjs/splunk"], function (Backbone, jquery, splunk_js_sdk) {
    var SetupView = Backbone.View.extend({
        ...
        render: async function () {
            ...
            Object.keys(this.current_settings["lookups"]).forEach((name) => {
                if (!name.includes("Cleanup")) {
                    ...
                    const matches = this.current_settings["lookups"][name]["search"].match(
                        /(.*) \| lookup sekoia_iocs_\w+ _key as (\w+)/
                    );
        ...
        parse_form: function parse_form() {
            ...
            const api_key = values[0].value.trim();         
        ...
        perform_setup: async function perform_setup(splunk_js_sdk, settings) {
            try {
                ...
                if (error !== null && typeof error === "object" && error.hasOwnProperty("responseText")) {
                    var response_object = JSON.parse(error.responseText);
                    error_messages_to_display = this.extract_error_messages(response_object.messages);
                } else {
    ...
    return SetupView;
}); // End of require asynchronous module definition function // End of require statement

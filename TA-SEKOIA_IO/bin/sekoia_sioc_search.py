...
class SIOCSearchCommand(StreamingCommand):
    ...
    def generate_search_job(self, record):
        try:
            ...
            searchquery = record.get("x_pattern_splunk")
            if not searchquery:
                raise NoSplunkPatternFoundError()

            # inject the index filtering in the query
            searchquery = searchquery.replace(" earliest=", " index=test earliest=")

            # extend the search query to report the result in kvstore
            searchquery += (
                ', index | stats values(index) as match_indexes earliest(_time) as match_first_seen latest(_time) as match_last_seen count as match_count | eval indicator_id="'
                + str(record["id"])
                + '" |  lookup sioc_lookup indicator_id AS indicator_id OUTPUTNEW | outputlookup sioc_lookup append=True key_field=_key max=1'
            )
            ...
            self.report_search(job_id=job.name, record=record)
    ...
    def report_search(self, job_id, record):
        ...
        kv_fields = {
            ...
            "indicator_kill_chain_phases": [
                str(killchain.get("kill_chain_name")) + ":" + str(killchain.get("phase_name"))
                for killchain in indicator.get("kill_chain_phases", [])
            ],
            ...
            "indicator_x_pattern_splunk": record.get("x_pattern_splunk"),
            "indicator_x_inthreat_sources_refs": indicator.get("x_inthreat_sources_refs"),
        }

        res = collection.data.insert(json.dumps(kv_fields))
        self._logger.info(
            "Search report inserted in collection " + str(collection_name) + ": " + str(res)
        )
    ...
    def stream(self, records):
        ...
        for record in records:
            if nb_triggered_search >= max_search_per_run:
                self._logger.warn(
                    "Max number of search per run reached, other searches will be processed in next run"
                )
            else:
                try:
